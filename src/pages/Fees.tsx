...
import { CreditCard, Download, History, Wallet, AlertTriangle, Printer, Landmark, ShieldAlert, Smile, Loader2 } from "lucide-react";
import { authApi } from "@/lib/api";

interface FeePaidRecord {
    date: string;
    mode: string;
    number: string;
    amount: number;
...
export default function Fees() {
  ...
  const handleFakePayment = () => {
    const pendingAmount = data?.due.totalDue || 0;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-muted-foreground animate-pulse">Syncing with ERP Financials...</p>
      </div>
    );
  }
    ...
    return (
      <div className="p-8">
        <Alert variant="destructive">
          ...
          <AlertDescription>{error || "Could not load fee data."}</AlertDescription>
        </Alert>
  ...
  const progressPct = totalFee > 0 ? Math.round((paidAmount / totalFee) * 100) : 0;

  const mainDueItem = due.list.length > 0 
    ? due.list[0] 
    : { head: "No Active Dues", amount: 0, dueDate: "—" };
  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-500 max-w-7xl mx-auto">
            ...
            <Card className="bg-gradient-to-br from-red-50 to-rose-100/50 dark:from-red-900/10 dark:to-transparent border-red-200 dark:border-red-800 shadow-sm">
                ...
                <CardFooter>
                     <p className="text-xs text-red-600/80 dark:text-red-400/80 flex items-center gap-1 font-semibold">
                        <AlertTriangle className="w-3 h-3" /> {pendingAmount > 0 ? `Due by ${mainDueItem.dueDate}` : "No pending dues"}
                     </p>
                </CardFooter>
            </Card>
        ...
        <Tabs defaultValue={pendingAmount > 0 ? "pay" : "history"} className="w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="lg:col-span-2 shadow-sm border-2 border-primary/10">
                            ...
                            <div className="p-4 bg-muted/40 rounded-lg space-y-3">
                                {due.list.length > 0 ? (
                                    ...
                                    <div className="py-4 text-center text-muted-foreground">
                                        ...
                                        <p className="text-sm">No fee dues found for this account.</p>
                                    </div>
                                )}
                            </div>
                        ...
                        <CardFooter className="flex flex-col md:flex-row gap-3 bg-muted/20 py-4">
                            <Button className="w-full md:w-auto text-base" size="lg" onClick={handleFakePayment} disabled={isPaying || pendingAmount <= 0}>
                    ...
                    <div className="space-y-4">
                        <Alert variant="destructive" className="bg-red-50 text-red-900 border-red-200 dark:bg-red-900/10 dark:text-red-300 dark:border-red-900">
                            ...
                            <AlertDescription className="text-xs leading-relaxed mt-2 list-disc pl-4 space-y-1">
                                ...
                                <li>Clear at least 50% of current Academic Year's fee to be eligible for Exam Fee generation.</li>
                            </AlertDescription>
                        </Alert>
            ...
            <TabsContent value="history" className="mt-4">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Live data synced from ERP Financials</CardDescription>
                    </CardHeader>
