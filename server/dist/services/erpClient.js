...
class ERPClient {
        ...
        this.client.interceptors.response.use(async (response) => {
            ...
            if (cookies) {
                ...
                for (const c of list) {
                    ...
                    try {
                        await this.jar.setCookie(c, this.baseUrl);
                    }
                    catch (e) { }
                }
            }
    ...
    async login(creds) {
        ...
        try {
            const res = await this.client.post(loginUrl, qs_1.default.stringify(payload));
    ...
    async getStudentProfile() {
        ...
        try {
            ...
            $('table.maintable tr').each((_, row) => {
                ...
                else if (label.includes('dob')) {
                    const parts = val.split('/');
                    data.dob = parts[0];
                    data.gender = parts[1]?.trim() || '';
                }
                else if (label.includes('aadhaar'))
                    data.aadhaar = val;
                else if (label.includes('father'))
                    data.father = val;
                else if (label.includes('residential'))
                    data.address = val;
                else if (label.includes('studentcontact')) {
                    const parts = val.split('/');
                    data.contact = parts[0];
                    data.email = parts[1]?.trim() || '';
                }
                else if (label.includes('parent'))
                    data.parentContact = val;
                ...
                else if (label === 'hosteller')
                    data.hosteller = val;
                else if (label.includes('income'))
                    data.income = val.replace('/', '').trim();
    ...
    async fetchImageStream(imageUrl) {
    ...
    async getStudentSubjects() {
        ...
        try {
            ...
            rows.each((i, row) => {
                ...
                if (sem.toLowerCase() === 'semester' || !sem.match(/^\d+$/))
                    return;
    ...
    async getAttendance() {
        ...
        try {
            ...
            $('#tblSubjectWiseAttendance tr').each((i, row) => {
                ...
                // Normal Data Rows
                if ($(row).attr('class') === 'subheader' || $(row).find('.header').length > 0)
                    return;
                if (cols.length < 6)
                    return;
    ...
    async getHourWiseAttendance() {
        ...
        try {
            ...
            const getHiddenVal = (id) => parseFloat($(`input[id='${id}']`).val()) || 0;
            const stats = {
            ...
            $('table[name="table1"] tr').each((_, row) => {
                ...
                $(row).find('td').each((idx, cell) => {
                    ...
                    // Handle "&nbsp;" or empty
                    if (!status || status.charCodeAt(0) === 160)
                        status = "-";
                    hours.push(status);
                });
        ...
        try {
            ...
            subjectRows.each((_, row) => {
                ...
                detailTable.each((_, dRow) => {
                    ...
                    // Validation
                    if (!obt)
                        obt = "-"; // Placeholder for empty values
                    components.push({
                        label,
                        obtained: obt,
                        max: mx
    ...
    async getFees() {
        ...
        try {
            ...
            $paid('#tblFinanceDetails tr').each((_, row) => {
                ...
                if (cols.length < 4 || $paid(row).hasClass('header') || $paid(row).hasClass('subheader'))
                    return;
            ...
            $due('#tblFeeDueDetails tr').each((_, row) => {
                if ($due(row).hasClass('header') || $due(row).hasClass('subheader') || $due(row).hasClass('subheader1'))
                    return;
